 ...
 * TracingConstants#PRINCIPAL_TRACE_ID_FIELD} in the {@link java.security.Principal} object, and use this field as traceId.
 * </p>
 * <pre>
 *     Snippet:
 *     {@code
...
public class SecurityContextTraceIdExtractor implements TraceIdExtractor {
    ...
    public String extractTraceId() {
        ...
        if (auth != null && auth.getPrincipal() != null) {
            ...
            Map<String, String> map = objectMapper.convertValue(auth.getPrincipal(), Map.class);
            if (map.containsKey(TracingConstants.PRINCIPAL_TRACE_ID_FIELD)) {
                traceId = String.valueOf(map.get(TracingConstants.PRINCIPAL_TRACE_ID_FIELD));
            } else {
                logger.warn("Principal object doesn't contain field {}, unable to extract traceId", TracingConstants.PRINCIPAL_TRACE_ID_FIELD);
            }
        } else {
